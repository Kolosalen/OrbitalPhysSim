...
def analyze_results(system, results):
    ...
    ax3.set_title('Ошибка в позиционировании за сутки')
    ax3.set_xlabel('Спутниковая система')
    ax3.set_ylabel('Ошибка (метры/день)')
    ...
    for sat_name, result in results.items():
        ...
        # Вычисляем суточную ошибку положения
        one_day_idx = np.argmax(t_newton > 86400)
        if one_day_idx > 0:
    ...
    # 3. Ошибка в определении положения за день
    if daily_position_errors:
        sat_names = list(daily_position_errors.keys())
        error_values = list(daily_position_errors.values())
        bar_colors = [colors[name] for name in sat_names]

        bars = ax3.bar(range(len(sat_names)), error_values,
        ...
        for bar, value in zip(bars, error_values):
            ...
            ax3.text(bar.get_x() + bar.get_width() / 2, height * 1.02,
                     f'{value:.2f} м', ha='center', va='bottom', fontsize=9)
    ...
    plt.suptitle('Лабораторная работа: Влияние ОТО на точность спутниковых навигационных систем', # Поправить название НАДОНАДОНАДО
...
def demonstrate_gps_effects():
    """
    Демонстрация эффектов для GPS систем
    """
    ...
    # Вывод результатов
    print(f"\nСравнение релятивистских эффектов для разных систем:")
    ...
    print("\n" + "=" * 80)
    print("Ключевые выводы для GPS:")
    print("=" * 80)

    gps_result = [r for r in results if r['name'] == 'GPS'][0]

    print(f"\n1. Параметры GPS:")
    print(f"   Высота орбиты: {gps_result['height']:.0f} км")
    print(f"   Скорость: {gps_result['velocity']:.2f} км/с")
    print(f"   Гравитационный эффект: +{gps_result['delta_grav'] * 1e9:.2f} нс/сек")
    print(f"   Кинематический эффект: {gps_result['delta_kin'] * 1e9:.2f} нс/сек")
    print(f"   Суммарный эффект: +{gps_result['delta_total'] * 1e9:.2f} нс/сек")
    ...
    print(f"   Без поправок GPS давал бы ошибку ~{gps_result['positioning_error'] / 1000:.1f} км/день")
    print(f"   Реальные системы GPS включают поправку:")
    print(f"   - Часы на спутниках настроены на 10.22999999543 МГц вместо 10.23 МГц")
    print(f"   - Это замедляет часы на {gps_result['delta_total']:.3e} от номинальной частоты")

    print(f"\n4. Для других систем:")
    print(f"   Galileo: выше орбита → больший гравитационный эффект")
    print(f"   ГЛОНАСС: ниже орбита → меньший эффект, но всё ещё значительный")
    print(f"   Низкие орбиты: эффекты пренебрежимо малы для навигации")
...
def main():
    ...
    print(f"\nОшибки в позиционировании за 1 день (моделирование):")
    for sat_name, error in daily_pos_errors.items():
        print(f"  {sat_name}: {error:.2f} м")


    plt.show()

    return relativistic_system, all_results, daily_pos_errors


if __name__ == "__main__":
    # Запускаем основную программу
    system, results, errors = main()